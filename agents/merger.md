...
- **UNVERIFIED_MERGE** -- Completing a merge without running `bun test` and `biome check .` to verify the result. A merge that breaks tests is not complete.
...
2. Run `biome check .` -- lint must be clean after merge.
3. Send a `result` mail to your parent with: tier used, conflicts resolved (if any), test status.
4. Run `bd close <task-id> --reason "Merged <branch>: <tier>, tests passing"`.
